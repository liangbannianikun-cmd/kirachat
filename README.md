...
<h1 align="center">澄语 KiraChat</h1>

<p align="center"><strong>把 SillyTavern 角色卡、多模型 API、本地模型和群聊带到原生 Android / iOS 客户端。</strong></p>
...
<p align="center">
  <a href="https://github.com/liangbannianikun-cmd/kirachat/releases/download/v0.9.0/KiraChat-0.9.0-android-debug.apk"><strong>下载 Android APK</strong></a>
  ·
  <a href="https://github.com/liangbannianikun-cmd/kirachat/releases/download/v0.9.0/KiraChat-0.9.0-ios-unsigned.ipa"><strong>下载 iOS IPA</strong></a>
  ·
  <a href="https://github.com/liangbannianikun-cmd/kirachat/releases/tag/v0.9.0">版本与校验文件</a>
</p>
...
KiraChat 安装包与校验文件统一发布在 **[KiraChat v0.9.0 Release](https://github.com/liangbannianikun-cmd/kirachat/releases/tag/v0.9.0)**。安装或覆盖前建议先在“我的 → 备份与还原”导出数据。
...
- Tavern V1/V2/V3 JSON 角色卡导入
- PNG 角色卡导入：兼容旧 `chara` 与 Character Card V3 `ccv3` 元数据，并保留原图 IDAT 像素作为头像
- `{{char}}`、`{{user}}`、`<BOT>`、`<USER>` 宏替换
- 角色首句、描述、性格、场景和示例对话提示词组合
- 角色卡内嵌 `character_book` 世界书；支持最多 1,000,000 条，并按最近消息关联性在上下文安全预算内动态选取
...
- 群聊详情页支持修改群聊名称和批量拉入本地角色；成员变化会立即同步到组合头像、后续回复范围和本地备份
- “我的 → 备份与还原”可导出或完整还原角色、群聊、聊天记录、世界书、头像、聊天背景与普通设置；API Key 和 OAuth 令牌不会进入备份
- “我的 → 服务器同步”可连接自部署服务，在 Android 与 iOS 之间自动同步通用快照；上传前使用客户端 AES-GCM 加密，修订冲突会暂停并要求明确选择保留版本
...
- 非内置角色可从资料页直接更换 JSON/PNG 角色卡，保留原聊天记录、群聊关系和会话设置
...
8. 如需多设备同步，先部署 [`sync-server`](sync-server/README.md)，再在“我的 → 服务器同步”填写服务器地址、同步令牌和单独的加密密码。第一台设备选择“上传本机”，其他设备选择“下载服务器”，之后即可开启自动同步。

应用允许连接局域网 HTTP 兼容接口。请只在可信局域网使用 HTTP；公网 API 必须使用 HTTPS。

## 服务器同步

仓库内置无第三方运行时依赖的 Node.js 同步服务：
...
公网部署必须由反向代理提供 HTTPS。同步令牌负责服务器访问控制；端到端加密密码只在客户端使用，不会发送给服务器。服务器仅保存密文、修订号、更新时间和设备类型。角色、群聊、消息、世界书、头像、聊天背景、用户名及普通功能开关会同步；API Key、GPT/Copilot OAuth 令牌、Realtime 凭据和本地模型文件不会同步。

自动同步采用条件写入：如果服务器与本机都在上次同步后发生变化，客户端不会自动覆盖，而会提示通过“上传本机”或“下载服务器”选择保留版本。
